...
| **Redis** | Message deduplication, rate limiting state |
...
| Retry | Max 3 attempts, exponential backoff (2s, 4s) |
...
| conversation_key | VARCHAR(256) | UNIQUE |
...
- **Headers:** `X-Zalo-Signature` (HMAC-SHA256), `Content-Type: application/json`
- **Behavior:**
  1. Verify HMAC-SHA256 signature against raw body
  2. Parse body → extract `sender.id`, `message.message_id`, `message.text`
  3. Check Redis for `message_id` (TTL 24h) → dedupe
  4. Store `message_id` in Redis with 24h TTL
  5. Publish to `conversation.process` queue
  6. Return `{"success": true}` immediately

#### GET /health/live
Returns `{"status": "alive"}` — no dependency checks.

#### GET /health/ready
Verifies PostgreSQL, Redis, RabbitMQ connections.
- Returns `200 OK` with `{"status": "ready", "checks": {...}}` if all pass
...
| POST /internal/replay | Re-queue conversation for reprocessing |
...
- **Max attempts:** 3
- **Backoff:** Exponential — 2s, 4s (base=2)
...
4. Build prompt: active system prompt + last 10 messages
5. Call Anthropic Claude API (timeout 15s)
6. Parse response:
...
2. Get Zalo access token from config
3. Call ZaloClient.send_text(user_id, text)
...
/app
  ...
  /workers
    /conversation
      __init__.py
      main.py             # Worker entry, signal handling
      consumer.py         # RabbitMQ consumer loop
      processor.py        # Core pipeline (LLM + tools)
      llm.py              # Anthropic client wrapper
      tools.py            # Tool registry + executors
      prompts.py          # Prompt loading + caching
    /outbound
      __init__.py
      main.py
      consumer.py
      processor.py        # Zalo sending + retry logic
      zalo_client.py      # Zalo API client wrapper
    /shared
      __init__.py
